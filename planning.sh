...
do
    ...
    then
        ...
        tail -n +2 "$fichier_csv" | sort -t';' -k3,3 -k6,6 | while IFS=';' read id tache priorite duree categorie echeance statut
        do
            fin=$date_courante

            fin_sec=$(date -d "$fin" +%s)
            echeance_sec=$(date -d "$echeance" +%s)

            if [ "$fin_sec" -gt "$echeance_sec" ]; then
                echo "Impossible : $tache (échéance $echeance)"
                impossible=1
                break
            fi

            echo "$date_courante : $tache"
            date_courante=$(date -d "$date_courante +1 day" +%Y-%m-%d)
        done
    elif [ "$choix" -eq 5 ]
    then
        echo "Au revoir !"
